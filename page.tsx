...
import { OUTPUTS } from '@/components/SideNav';
import TopBar from '@/components/TopBar';
import { getInstance, getInstances, getSections, getProgress } from '@/lib/db';
import { getSessionUser } from '@/lib/auth';
import {
  getRegistry, getBudget, getSponsors, getMetrics, getSchedule,
  budgetTotals, budgetByMonth, sponsorTotals, money, fmtDate, fmtMonth,
} from '@/lib/outputs';
...
const TITLES: Record<string, { title: string; eyebrow: string; lede: string }> = {
  timeline: {
    title: 'Timeline', eyebrow: 'Derived output',
    lede: 'Every dated task for this event, rolled up by phase. Nothing here is entered twice — the dates come from the checklist, so the schedule updates itself as the playbook is worked.',
  },
  budget: {
    title: 'Budget', eyebrow: 'Derived output',
    lede: 'Revenue and expense lines for this event, with a month-by-month view of when the money actually moves.',
  },
  sponsors: {
    title: 'Sponsors & Exhibitors', eyebrow: 'Derived output',
    lede: 'Who has committed, what they were promised, and which of those promises have been delivered.',
  },
  narrative: {
    title: 'HRSA Narrative', eyebrow: 'Derived output',
    lede: 'A draft written from the registry, budget, sponsorship and outcome data on file. Read it, correct it, then paste it into the report.',
  },
  feed: {
    title: 'Exports', eyebrow: 'Derived output',
    lede: 'The same data in formats other systems can read — the website, a calendar, a spreadsheet.',
  },
};
...
export default async function OutputPage({
  ...
  const [instances, sections, progress, user, reg] = await Promise.all([
    getInstances(), getSections(instance.template_id),
    getProgress(instance.id), getSessionUser(), getRegistry(slug),
  ]);
  ...
  return (
    <>
      <TopBar
        ...
        section={`out/${view}`} editing={false} onToggleEdit={undefined}
      />
      <div className="wrap">
        <SideNav
          instanceSlug={slug} sections={sections} progress={progress}
          active={`out:${view}`}
        />
        <main>
...
async function TimelineView({ instance }: { instance: { id: string; template_id: string } }) {
  ...
  if (!tasks.length) {
    return (
      <p className="empty">
        No due dates set for this event yet. Add them from the ⚑ button on any checklist item and
    ...
    <>
      ...
      <div className="card">
        ...
        <p style={{ fontSize: '.8rem', color: 'var(--muted)', margin: '.6rem 0 0' }}>
          {tasks.filter((t) => !t.done).length} open · {tasks.filter((t) => t.done).length} complete
          · {tasks.length} dated tasks in total
        </p>
      </div>
    </>
  );
...
async function BudgetView({ instanceId, target }: { instanceId: string; target: number | null }) {
  ...
  const cats = (kind: 'revenue' | 'expense') => {
    ...
    for (const l of lines.filter((x) => x.kind === kind)) {
      (m.get(l.category) ?? m.set(l.category, []).get(l.category)!).push(l);
    }
    return [...m.entries()];
  };
  ...
  return (
    <>
      <div className="stats">
        ...
        <div className="stat">
          <div className="n" style={{ color: t.netP >= 0 ? 'var(--ok)' : 'var(--risk)' }}>{money(t.netP)}</div>
          <div className="l">Planned net</div>
        </div>
        <div className="stat">
          <div className="n">{target ? money(target) : '—'}</div>
          <div className="l">Revenue target</div>
        </div>
      </div>
      ...
      {(['revenue', 'expense'] as const).map((kind) => (
        <div className="card" key={kind}>
          ...
          <div className="tw">
            <table>
              <thead>
                <tr><th>Line</th><th style={{ width: 120 }}>When</th>
                  <th style={{ width: 120, textAlign: 'right' }}>Planned</th>
                  <th style={{ width: 120, textAlign: 'right' }}>Actual</th></tr>
              </thead>
              <tbody>
                    ...
                    {rows.map((l) => (
                      <tr key={l.id}>
                        ...
                        <td style={{ color: 'var(--muted)', whiteSpace: 'nowrap' }}>
                          {l.month ? fmtMonth(l.month.slice(0, 7)) : '—'}
                        </td>
                        <td style={{ textAlign: 'right' }}>{money(l.planned)}</td>
                        <td style={{ textAlign: 'right', color: l.actual == null ? 'var(--muted)' : undefined }}>
                          {l.actual == null ? '—' : money(l.actual)}
                        </td>
                      </tr>
                    ))}
                  </Fragment>
      ...
      <div className="card">
        ...
        <p style={{ fontSize: '.86rem', color: 'var(--muted)', marginTop: 0 }}>
          When each planned line lands. Useful for cash timing — the big F&amp;B and AV commitments
          hit the month before the event, while most registration revenue arrives earlier.
        </p>
        <div className="tw">
          <table>
            <thead>
              ...
              {months.map((m) => {
                ...
                return (
                  <tr key={m.month}>
                    ...
                    <td style={{ textAlign: 'right', color: m.net >= 0 ? 'var(--ok)' : 'var(--risk)' }}>
                      {money(m.net)}
                    </td>
                    <td>
  ...
  return (
    <>
      <div className="stats">
        ...
        <div className="stat">
          <div className="n">{t.benefitsDone}/{t.benefitsTotal}</div>
          <div className="l">Promised benefits delivered</div>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="tw">
          <table>
            <thead>
              <tr><th>Organisation</th><th>Tier</th><th style={{ textAlign: 'right' }}>Amount</th>
                <th>Status</th><th style={{ width: '34%' }}>Benefits</th></tr>
            </thead>
            <tbody>
              {sponsors.map((s) => (
                <tr key={s.id} className={s.status === 'declined' ? 'muted' : ''}>
                  <td>
                    {s.org}
                    {s.contact_name && <div className="sub">{s.contact_name}</div>}
                  ...
                  <td>
                    {s.benefits.length ? (
                      <ul className="bl">
                        {s.benefits.map((b, i) => (
                          <li key={i} className={b.done ? 'y' : 'n'}>{b.done ? '✓' : '○'} {b.label}</li>
                        ))}
                      </ul>
...
function FeedView({ slug }: { slug: string }) {
  const rows: [string, string, string][] = [
    ['Event feed (JSON)', `/api/i/${slug}/feed.json`,
      'Name, dates, venue, registration window, progress and sponsor logos — what a website or app would consume.'],
    ['Schedule (iCal)', `/api/i/${slug}/schedule.ics`,
      'Every dated task as a calendar event. Subscribe in Outlook or Google Calendar.'],
    ['Budget (CSV)', `/api/i/${slug}/budget.csv`,
      'Every revenue and expense line, for Excel or the finance team.'],
    ['Sponsors (CSV)', `/api/i/${slug}/sponsors.csv`,
      'Sponsor list with tier, amount, status and benefit fulfilment.'],
  ];
  return (
    <div className="card">
      ...
      <p style={{ fontSize: '.83rem', color: 'var(--muted)' }}>
        These require a signed-in session. For a public website feed the JSON endpoint would need a
        read-only token — worth doing once someone actually wires it to ncchca.org.
      </p>
    </div>
  );
}
...
async function narrativeData(instanceId: string, templateId: string) {
  const [lines, sponsors, metrics, sched] = await Promise.all([
    getBudget(instanceId), getSponsors(instanceId), getMetrics(instanceId),
    getSchedule(instanceId, templateId),
  ]);
  return {
