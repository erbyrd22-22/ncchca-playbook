...
/**
 * The sidebar. Playbook sections come from the database; the Outputs group
 * is fixed, because those pages are derived views rather than editable content.
 */
export default function SideNav({
  instanceSlug, sections, progress, active,
}: {
  instanceSlug: string;
  sections: Section[];
  progress: Record<string, { total: number; done: number; pct: number | null }>;
  /** Section slug, or `out:<key>` for one of the output pages. */
  active: string;
}) {
    ...
    <nav className="side">
        ...
        <div key={g}>
          ...
          {list.map((s) => {
            ...
            return (
              <a
                key={s.id}
                href={`/i/${instanceSlug}/${s.slug}`}
                className={s.slug === active ? 'on' : ''}
              >
      ...
      <div>
        ...
        {OUTPUTS.map((o) => (
          <a
            key={o.key}
            href={`/i/${instanceSlug}/out/${o.key}`}
            className={active === `out:${o.key}` ? 'on' : ''}
          >
