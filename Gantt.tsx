...
/** Days between two ISO dates, computed in UTC so it never shifts by timezone. */
...
/**
 * A Gantt chart drawn with plain CSS — one row per phase, positioned as a
 * percentage of the total span. No charting library, so nothing to keep
 * up to date and nothing extra to ship to the browser.
 */
export default function Gantt({ bars }: { bars: Bar[] }) {
  ...
  // Pad out to whole months so the column headers line up.
  ...
  const gridEnd = `${addMonth(endMonth, 1)}-01`;
  const span = Math.max(days(gridStart, gridEnd), 1);

  const pct = (iso: string) => (days(gridStart, iso) / span) * 100;

  // Group the rows under their section heading.
  const groups: { section: string; rows: Bar[] }[] = [];
  for (const b of bars) {
    const g = groups.find((x) => x.section === b.section);
    if (g) g.rows.push(b);
    else groups.push({ section: b.section, rows: [b] });
  }
  ...
  return (
    <div className="card">
      ...
      <div className="gwrap">
        <div className="gscroll">
          <div className="ghead" style={{ gridTemplateColumns: `repeat(${months.length}, 1fr)` }}>
            {months.map((m) => (
              <div className="gm" key={m}>
                {MON[+m.slice(5, 7) - 1]}
                <span>{m.slice(2, 4)}</span>
              </div>
            ))}
          </div>
                ...
                return (
                  <div className="grow" key={b.key}>
                    ...
                    <div className="gtrack">
                      {months.map((m, i) => (
                        <span className="ggl" key={m} style={{ left: `${(i / months.length) * 100}%` }} />
                      ))}
                      <div
                        className={`gbar ${complete === 100 ? 'done' : ''}`}
                        style={{ left: `${left}%`, width: `${width}%` }}
                        title={`${b.phase}: ${b.done}/${b.total} complete`}
                      >
      ...
      <p style={{ fontSize: '.8rem', color: 'var(--muted)', margin: '.7rem 0 0' }}>
        Each bar runs from the first to the last due date in that phase; the solid fill is how much
        of it is checked off. Change a due date on the checklist and this moves with it.
      </p>
    </div>
  );
}
